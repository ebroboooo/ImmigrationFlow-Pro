...
import { Users, Plus, Search, X, Globe, CreditCard, FileText, Phone, Mail } from 'lucide-react';
...
export const Clients = () => {
  ...
  if (loading) return <div className="p-8 text-gray-500 dark:text-gray-400">Loading clients...</div>;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white flex items-center gap-2">
            <Users className="w-6 h-6 text-indigo-500" /> Client Management
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">{clients.length} total clients · {clients.filter(c => c.immigrationStatus === 'Active Case').length} active cases</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="px-5 py-10 text-center text-gray-500 dark:text-gray-400">No clients found.</td></tr>
              ) : filtered.map(client => (
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="flex gap-3 p-6 border-t border-gray-200 dark:border-gray-800">
              <button onClick={() => setShowModal(false)} className="flex-1 px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">Cancel</button>
              <button onClick={handleCreate} className="flex-1 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors">Add Client</button>
            </div>
          </div>
        </div>
      )}
